...
opt_param = io.loadmat(FN_load+'opt_param_STB_weight.mat')
...
for trial in range(num_trials):
    ...
    if save_neighs:
        nei[trial, :] = np.sum(W>0, axis=1)

KME_error['Error'] = np.mean(kme_error, axis=0)
KME_error['std']   = np.std(kme_error, axis=0)
# save the results after each setting
io.savemat(FN_save+'KME_error_STB_weight.mat', KME_error)
if save_neighs:
        NEI = np.mean(nei, axis=0)
        np.savetxt(FN_save+'Neighbors_STB_weight.csv', NEI, delimiter=',')
print('Done')