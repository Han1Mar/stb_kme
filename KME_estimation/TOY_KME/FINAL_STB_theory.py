...
opt_param = io.loadmat(FN_load+'opt_param_STB_theory.mat') # 'Zeta', 'Gamma'
...
for s_idx in range(num_settings):
    ...
    for trial in range(num_trials):
        ...
        W  = u.compute_STB_weight(W, 'theory', zeta, gamma)
    ...
    io.savemat(FN_save+'KME_error_STB_theory.mat', KME_error)
    if save_neighs:
        NEI[s_idx] = np.mean(nei)
        np.savetxt(FN_save+'Neighbors_STB_theory.csv', NEI, delimiter=',')
print('Done')